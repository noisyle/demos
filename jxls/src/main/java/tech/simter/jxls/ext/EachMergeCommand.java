...
/**
 * 参考: https://github.com/simter/simter-jxls-ext
 * 改造为兼容旧版jxls
 */
public class EachMergeCommand extends EachCommand {
    ...
    public Size applyAt(CellRef cellRef, Context context) {
        ...
        List<Area> childAreas = this.getAreaList().stream().flatMap(area1 -> area1.getCommandDataList().stream())
                .flatMap(commandData -> commandData.getCommand().getAreaList().stream()).collect(Collectors.toList());
        List<AreaRef> childAreaRefs = childAreas.stream().map(Area::getAreaRef).collect(Collectors.toList());
    ...
    public static class MergeCellListener implements AreaListener {
        ...
        MergeCellListener(Transformer transformer, AreaRef parent, List<AreaRef> children, int parentCount) {
            ...
            int[] childCols = children.stream()
                    .flatMapToInt(
                            ref -> IntStream.range(ref.getFirstCellRef().getCol(), ref.getLastCellRef().getCol() + 1))
                    .distinct().sorted().toArray();

            // find all sub command start column
            this.childStartColumns = children.stream().mapToInt(ref -> ref.getFirstCellRef().getCol()).distinct()
                    .sorted().toArray();
        ...
        // remember that all sub command AreaListener is invoked before parent command
        // AreaListener.
        ...
        public void afterTransformCell(CellRef srcCell, CellRef targetCell, Context context) {
            ...
            if (targetCell.getCol() == parentStartColumn) { // main command process
                ...
                logger.debug("parent: srcCell={}, targetCell={} [{}, {}]", srcCell, targetCell, targetCell.getRow(),
                        targetCell.getCol());
                ...
                logger.debug("child: srcCell={}, targetCell={} [{}, {}]", srcCell, targetCell, targetCell.getRow(),
                        targetCell.getCol());
            }
        }
        ...
        private static void doMerge(Sheet sheet, List<int[]> records, int[] mergeColumns, CellRef srcCell) {
            if (logger.isDebugEnabled()) {
                logger.debug("merge: sheetName={}, records={}", sheet.getSheetName(), records.stream()
                        .map(startEnd -> "[" + startEnd[0] + "," + startEnd[1] + "]").collect(Collectors.joining(",")));
            }
        ...
        private static void merge4Row(Sheet sheet, int fromRow, int toRow, int[] mergeColumns, CellRef srcCell) {
            ...
            for (int col : mergeColumns) {
                ...
                if (originCell != null) {
                    ...
                    RegionUtil.setBorderTop(originCellStyle.getBorderTop(), region, sheet, sheet.getWorkbook());
                    RegionUtil.setBorderRight(originCellStyle.getBorderRight(), region, sheet, sheet.getWorkbook());
                    RegionUtil.setBorderBottom(originCellStyle.getBorderBottom(), region, sheet, sheet.getWorkbook());
                    RegionUtil.setBorderLeft(originCellStyle.getBorderLeft(), region, sheet, sheet.getWorkbook());
                } else {
                    RegionUtil.setBorderTop(BorderStyle.THIN.ordinal(), region, sheet, sheet.getWorkbook());
                    RegionUtil.setBorderRight(BorderStyle.THIN.ordinal(), region, sheet, sheet.getWorkbook());
                    RegionUtil.setBorderBottom(BorderStyle.THIN.ordinal(), region, sheet, sheet.getWorkbook());
                    RegionUtil.setBorderLeft(BorderStyle.THIN.ordinal(), region, sheet, sheet.getWorkbook());
                }
            }
        }
    }
